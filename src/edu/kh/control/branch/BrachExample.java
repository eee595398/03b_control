...
public class BrachExample {
	...
	public void ex2() {
		...
		int input = 0;
		int sum = 0;
		// while 문을 처음에 무조건 수행하고, 특정 조건에 종료하는 방법
		...
		public void ex3() {
			...
			while(true) {
				...
				// next() : 다음 한 단어(띄어쓰기 불가)
				// nextLine() : 다음 한줄(띄어쓰기 포함)
				
				if(input.equals("exit@")) {
					//String 자료형은 비교연산자(==)로 같은 문자열인지 판별할 수 없다.
					//비교 연산자는 보통 기본자료형끼리의 연산에서만 가능하다
		...
		public void ex4() {
			...
			for(int i=2; i<=9; i++) {
				for(int num = 1; num<=9; num++) {
					System.out.printf(" %d x %d =%2d",i, num, i*num);
		...
		public void RSPGame() {
			...
			for(int i =1; i<=round; i++) { // 입력 받은 판 수 만큼 반복
				...
				//Math.random();
				//난수 생성 -> 0.0이상 1.0 미만의 난수 생성
				
				// 1) 1~3사이 난수 생서
				// 2) 1이면 가위, 2이면 바위, 3이면 보지정
				...
				System.out.printf("현재 기록 : %d 승 %d무 %d패\n",win,draw,lose);
